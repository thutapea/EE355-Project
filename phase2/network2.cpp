...
// ─────────────────────────────────────────────
//  Helper: check if query matches any field of a person
//  Uses case-insensitive substring matching so partial
//  inputs (e.g. just the year, or part of an email) also work
// ─────────────────────────────────────────────

static bool wise_match(Person* p, const string& query) {
    string q = toLower(query);
    return toLower(p->get_phone()) == q ||
           toLower(p->get_email()) == q ||
           toLower(p->get_bdate()) == q ||
           toLower(p->get_fname()) == q ||
           toLower(p->get_lname()) == q;
}
...
void Network::showMenu() {
    ...
    while (1) {
        ...
        string fname, lname, fname2, lname2, fileName, bdate, email, phone;
        cout << "\033[2J\033[1;1H";
        ...
        // ── Option 6: Connect (Phase 2) ─────────────────────────
        else if (opt == 6) {
            cout << "Make friends \n";

            // ── Get Person 1 ──
            cout << "Person 1 First Name: ";
            getline(cin, fname);
            cout << "Last Name: ";
            getline(cin, lname);
            ...
            if (p1 == NULL) {
                ...
                if (p2 == NULL) {
                    ...
                    // ── Both exist: make the connection (double sided) ──
                    p1->addFriend(p2);  // p1 → p2
                    p2->addFriend(p1);  // p2 → p1
        ...
        // ── Option 7: Wise Search ───────────────────────────────
            ...
            Person* ptr = head;
            while (ptr != NULL) {
                // wise_match checks phone, email, and birth date
                // supports multiple matches (e.g. same birth date)
                // uses case-insensitive substring matching
                if (wise_match(ptr, query)) {
                    ptr->print_person();
                    cout << "------------------------------" << endl;
                    found = true;
                }
...
//Compile: g++ -o test_network test_network.cpp network2.cpp person.cpp date.cpp misc.cpp
//run: ./test_network